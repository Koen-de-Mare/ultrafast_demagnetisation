...
num_frames: int = round(150 / simpar.dt)
...
steps = [x * simpar.sliceLength for x in range(0, system.num_slices)]
...
ax1.set_xlim(0, simpar.sliceLength * system.num_slices)
...
ax2.set_xlim(0, simpar.sliceLength * system.num_slices)
...
ax3.set_xlim(0, simpar.sliceLength * system.num_slices)
...
ax4.set_xlim(0, simpar.sliceLength * system.num_slices)
...
def animate(n):
    ...
    line2.set_data(steps, list(map((lambda x: x / syspar.Ds_up), excited_up_lists[n])))
    line3.set_data(steps, mu0_dn_lists[n])
    line4.set_data(steps, list(map((lambda x: x / syspar.Ds_dn), excited_dn_lists[n])))

    magnetisation = [0.0] * system.num_slices
    for i in range(system.num_slices):
        magnetisation_thermal = \
            mu0_up_lists[n][i] * syspar.Ds_up - mu0_dn_lists[n][i] * syspar.Ds_dn
...
anim = animation.FuncAnimation(fig, animate, init_func=init, frames=num_frames, interval=simpar.dt*100, blit=True)
